...
class AtomExecutor(BaseExecutor):
    ...
    def get_att_file(self, contents, articleUrl, urlPre1):
        ...
        #         #判断是否有超链接
        #         if line.xpath('.//a') != [] and ''.join(line.xpath('.//a//text()')) != '':
        #             text = ''.join(line.xpath('.//a//text()'))
        #             oldTexts.append(text)
        #             url = ''.join(line.xpath('.//a/@href'))
        #             #判断超链接是否需要补全
        #             if 'http' not in url:
        #                 urlPre = re.sub('[^/]+(?!.*/)', '', articleUrl)
        #                 if '../../'in url:
        #                     urlPre1 = re.sub('[^/]+(?!.*/)', '', urlPre1)
        #                     url = url.replace('../../', urlPre1)
        #                 else:
        #                     url = url.replace('./', urlPre)
        #             Urls.append(url)
    ...
    def get_detail_data(self, singleObj, types, urlPre, domain, wd):
        ...
        # 文章标题
        articleTitle = singleObj.xpath('.//ul/li/a/@title')
        # print(articleTitle)
        # 文章链接
        articleUrl = singleObj.xpath('.//ul/li/a/@href')
        article_list = []
        for i in articleUrl:
            if 'http' in i:
                article_list.append(i)
        for articleUrl, articleTitle in zip(article_list, articleTitle):
            # print(articleUrl, articleTitle)
            contentSource = self.request_get(url=articleUrl).text
            contentHtmlElement = etree.HTML(contentSource)
            articleId = hashlib.md5(articleUrl.encode(encoding='utf-8')).hexdigest()
            issueNumber = str(re.findall('\[字号\]>begin-->(.*?)<', contentSource))
            articleSource = str(re.findall('\[发文单位\]>begin-->(.*?)<', contentSource))
            contents = contentHtmlElement.xpath('//div[@class="info-cont zoom"]/p/text()')
            contents = self.get_content(contents, articleUrl)
            # print(contents)
            # print(articleTitle)
            # 正文内容
            publishDate_pre = contentHtmlElement.xpath('//meta[@name="PubDate"]')
            if publishDate_pre:
            ...
            if contentHtmlElement.xpath('//iframe[contains(@class,"video")]') != []:
                ...
                contents = f"<video>{video}</video>"
            # 正文不为视频
            else:
                contents = self.get_content(contents, urlPre)
    ...
    def crawl_thread(self, page, urlPre, domain, mycol, types, wd):
        while True:
            ...
            list = listHtmlElement.xpath('/html')
            # print(list)
            # 判断当前页是否有内容
            # if list == []:
            if page == 1:
